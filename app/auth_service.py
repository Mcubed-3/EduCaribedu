...
PLAN_LIMITS = {
    "free": {
        ...
        "monthly_activity_generations": 0,
    },
    "pro": {
    ...
    "plus": {
        ...
        "monthly_activity_generations": None,
    },
    "admin": {
        "monthly_generations": None,
        "saved_lessons": None,
...
def init_auth_db() -> None:
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS monthly_usage (
            ...
            activity_generation_count INTEGER NOT NULL DEFAULT 0,
    ...
    usage_cols = {row["name"] for row in cur.execute("PRAGMA table_info(monthly_usage)").fetchall()}
    if "activity_generation_count" not in usage_cols:
        cur.execute("ALTER TABLE monthly_usage ADD COLUMN activity_generation_count INTEGER NOT NULL DEFAULT 0")
...
def _normalize_plan(role: str, plan: str) -> str:
    if role == "admin":
        return "admin"
    return plan if plan in {"free", "pro", "plus"} else "free"
...
def create_user(
    ...
    cur.execute(
        ...
        INSERT INTO users (
            ...
            subscription_status, payment_provider, stripe_customer_id, stripe_subscription_id,
            subscription_started_at, subscription_renews_at, billing_notes, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            email, password_hash, salt, role, plan, subscription_status,
...
def list_users() -> List[Dict[str, Any]]:
    ...
    rows = conn.execute(
        """
        SELECT id, email, role, plan, subscription_status, payment_provider,
...
def update_user_role_plan(user_id: int, role: str, plan: str) -> Optional[Dict[str, Any]]:
    ...
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE users SET role = ?, plan = ?, subscription_status = ? WHERE id = ?", (role, plan, subscription_status, user_id))
    conn.commit()
...
def update_user_plan(user_id: int, plan: str) -> Optional[Dict[str, Any]]:
    ...
    conn = _get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE users SET plan = ?, subscription_status = ? WHERE id = ?", (plan, subscription_status, user_id))
    conn.commit()
    updated = cur.rowcount > 0
    conn.close()
    return get_user_by_id(user_id) if updated else None
...
def update_user_billing(
    ...
    cur.execute(
        """
        UPDATE users SET role = ?, plan = ?, subscription_status = ?, payment_provider = ?,
            stripe_customer_id = ?, stripe_subscription_id = ?, subscription_started_at = ?,
            subscription_renews_at = ?, billing_notes = ? WHERE id = ?
        """,
        (role, plan, subscription_status, payment_provider.strip(), stripe_customer_id.strip(), stripe_subscription_id.strip(), subscription_started_at.strip(), subscription_renews_at.strip(), billing_notes.strip(), user_id),
    )
    conn.commit()
    updated = cur.rowcount > 0
    conn.close()
...
def update_user_stripe_subscription(
    ...
    if not user:
        return None
    next_plan = plan or user.get("plan", "free")
    if next_plan not in {"pro", "plus", "admin"}:
        next_plan = "pro"
...
def cancel_user_paid_plan(user_id: int, note: str = "") -> Optional[Dict[str, Any]]:
    ...
    return update_user_billing(
        ...
        payment_provider=user.get("payment_provider", "stripe"),
        ...
        billing_notes=note or user.get("billing_notes", ""),
    )


def find_user_by_stripe_customer_id(customer_id: str) -> Optional[Dict[str, Any]]:
    conn = _get_conn()
    row = conn.execute("SELECT * FROM users WHERE stripe_customer_id = ?", (customer_id.strip(),)).fetchone()
    conn.close()
    return dict(row) if row else None
...
def find_user_by_stripe_subscription_id(subscription_id: str) -> Optional[Dict[str, Any]]:
    conn = _get_conn()
    row = conn.execute("SELECT * FROM users WHERE stripe_subscription_id = ?", (subscription_id.strip(),)).fetchone()
    conn.close()
    return dict(row) if row else None
...
def verify_user(email: str, password: str) -> Optional[Dict[str, Any]]:
    ...
    if hmac.compare_digest(user["password_hash"], _hash_password(password, user["salt"])):
        return _row_to_user(user)  # type: ignore[arg-type]
    return None
...
def create_session(user_id: int) -> str:
    ...
    conn = _get_conn()
    conn.execute("INSERT INTO sessions (token, user_id, created_at, expires_at) VALUES (?, ?, ?, ?)", (token, user_id, created_at.isoformat(), expires_at.isoformat()))
    conn.commit()
    conn.close()
    return token
...
def get_generation_count(user_id: int, month_key: Optional[str] = None) -> int:
    ...
    conn = _get_conn()
    row = conn.execute("SELECT generation_count FROM monthly_usage WHERE user_id = ? AND month_key = ?", (user_id, month_key)).fetchone()
    conn.close()
...
def increment_generation_count(user_id: int, month_key: Optional[str] = None) -> int:
    ...
    conn.execute(
        """
        INSERT INTO monthly_usage (user_id, month_key, generation_count, activity_generation_count)
        VALUES (?, ?, 1, 0)
...
def get_activity_generation_count(user_id: int, month_key: Optional[str] = None) -> int:
    month_key = month_key or get_month_key()
    conn = _get_conn()
    row = conn.execute("SELECT activity_generation_count FROM monthly_usage WHERE user_id = ? AND month_key = ?", (user_id, month_key)).fetchone()
    conn.close()
    return int(row["activity_generation_count"]) if row else 0
...
def increment_activity_generation_count(user_id: int, month_key: Optional[str] = None) -> int:
    ...
    conn.execute(
        ...
        VALUES (?, ?, 0, 1)
        ON CONFLICT(user_id, month_key)
        DO UPDATE SET activity_generation_count = activity_generation_count + 1
        """,
        (user_id, month_key),
    )
    conn.commit()
...
def can_generate_lessons(user: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if limit is None:
        return {"allowed": True, "used": used, "limit": None, "remaining": None}
    return {"allowed": used < limit, "used": used, "limit": limit, "remaining": max(limit - used, 0)}


def can_generate_activities(user: Dict[str, Any]) -> Dict[str, Any]:
    limits = get_plan_limits(user["plan"])
    if not limits.get("activity_generation", False):
        return {"allowed": False, "used": 0, "limit": 0, "remaining": 0}
    limit = limits["monthly_activity_generations"]
...
def can_save_more_lessons(user: Dict[str, Any], current_saved_count: int) -> Dict[str, Any]:
    ...
    if limit is None:
        return {"allowed": True, "used": current_saved_count, "limit": None, "remaining": None}
    return {"allowed": current_saved_count < limit, "used": current_saved_count, "limit": limit, "remaining": max(limit - current_saved_count, 0)}
...
def get_plan_status(user: Dict[str, Any], saved_lesson_count: int) -> Dict[str, Any]:
    ...
    activities = can_generate_activities(user)
    limits = get_plan_limits(user["plan"])
    return {
        "plan": user["plan"],
        "role": user["role"],
        ...
        "saved_lessons": saved,
        "activity_generations": activities,
        "activity_generation": limits["activity_generation"],
